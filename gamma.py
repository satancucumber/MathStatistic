...
intervals = [0, 1.6, 3.2, 4.8, 6.4, 24.0]   # объединенные интервалы
k = 5   # количество интервалов
k = len(intervals) - 1

lam = 0.14 #значение lambda_0
...
def p(i, lam):
    r = intervals[i + 1]
    l = intervals[i]
    return scipy.stats.gamma.cdf(x=r,a=0.5,scale=2/lam) - scipy.stats.gamma.cdf(x=l,a=0.5,scale=2/lam)
